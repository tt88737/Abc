...
<body>
  ...
  <script>
    ...
    function positionStage8YearPlan(source) {
      ...
      const years = asArray(positionStage8State.years)
    ...
    function positionStage8HistoryRows() {
      const years = asArray(positionStage8State.years).sort((a, b) => String(b.year || '').localeCompare(String(a.year || '')));
    ...
    function renderPositionStage8() {
      ...
      app.innerHTML = `<div class="grid">
        ...
        <section class="panel full"><h2>历史三阶段位置验证</h2><div class="table-scroll"><table class="compact-table"><thead><tr><th>年份</th><th>001-115</th><th>116-230</th><th>231-365</th><th>结论</th></tr></thead><tbody>${positionStage8HistoryRows()}</tbody></table></div></section>
      </div>`;
      document.getElementById('position-stage8-source')?.addEventListener('change', renderPositionStage8);
    }
    ...
    async function ensurePositionStage8Data() {
      if (positionStage8State?.years?.length) return positionStage8State;
