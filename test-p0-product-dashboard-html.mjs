...
for (const [name, text] of [["index.html", html], ["build-data.ps1", build]]) {
  ...
  assert.ok(text.includes("threeFormulaHomeSummary"), `${name} should summarize real three-in-three recommendation data`);
  ...
  assert.ok(text.includes("threeFormulaReviewSummary"), `${name} should summarize recent three-in-three review`);
